...
bool parseBitcoinURI(const QUrl &uri, SendCoinsRecipient *out)
{
    ...
    for (QList<QPair<QString, QString> >::iterator i = items.begin(); i != items.end(); i++)
    {
        ...
        else if (i->first == "amount")
        {
            if(!i->second.isEmpty())
            {
                if(!BitcoinUnits::parse(BitcoinUnits::XMC, i->second, &rv.amount))
                {
                    return false;
                }
            }
...
{
    return GetSpecialFolderPath(CSIDL_STARTUP) / "Bighancoin.lnk";
}

bool GetStartOnSystemStartup()
{
    // check for Bighancoin.lnk
...
HelpMessageBox::HelpMessageBox(QWidget *parent) :
    QMessageBox(parent)
{
    header = tr("Bighancoin-Qt") + " " + tr("version") + " " +
    ...
    setWindowTitle(tr("Bighancoin-Qt"));
